      ...
      <div className="w-full max-w-2xl flex items-center justify-between mb-12">
        ...
        <div className="flex gap-4">
          <div className="glass px-6 py-3 rounded-2xl text-white font-bold flex items-center gap-2">
            <Zap className="w-5 h-5 text-yellow-400" /> {score}
          </div>
          <div className={`glass px-6 py-3 rounded-2xl font-bold ${timeLeft < 10 ? 'text-rose-500 animate-pulse' : 'text-white'}`}>
            {timeLeft}s
          </div>
        </div>
      </div>

      <div className="glass-card p-12 rounded-[3.5rem] w-full max-w-2xl text-center relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary to-accent" />
        <h2 className="text-2xl font-display text-slate-400 mb-8 uppercase tracking-widest">Complète la suite</h2>
        ...
        <div className="flex justify-center gap-4 mb-12">
          {sequence.map((num, i) => (
            <motion.div
              ...
              className="w-16 h-16 md:w-20 md:h-20 glass rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-bold text-white border-white/10"
            >
              {num}
            </motion.div>
          ))}
          <div className="w-16 h-16 md:w-20 md:h-20 bg-primary/20 border-2 border-dashed border-primary/50 rounded-2xl flex items-center justify-center">
        ...
        <div className="grid grid-cols-2 gap-4">
          {options.map((opt, i) => (
            <motion.button
              ...
              className="py-6 glass rounded-2xl text-xl font-bold text-white hover:bg-white/5 transition-all border-white/5"
            >
              {opt}
            </motion.button>
          ))}
        ...
        {isGameOver && (
          ...
          >
            <div className="glass-card p-12 rounded-[3rem] text-center max-w-md w-full">
              ...
              <h2 className="text-4xl font-display text-white mb-4">Temps écoulé !</h2>
              <p className="text-slate-400 mb-8 text-lg">Ton score neural : <span className="text-primary font-bold">{score}</span></p>
              <button
